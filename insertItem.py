...
def insertPage():
    ...
    # Search Button
    if itemName and weight and quantity and price and category and store and int(quantity) > 0 and float(price) > 0 and float(weight) > 0:
        st.button('Add Item', on_click=insertItem, args=(itemName, store, price, weight, category, quantity))
    else:
        st.error("Please verify input", icon=None)
